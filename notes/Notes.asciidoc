...
The collected information till now suggest the following header / data format.

.Header format as C structure
----
struct part_hdr {
	uint8_t zero[4];
...
After the header follow the raw data with a size of content_size followed by a
four byte something (most likely some checksum).
...
Hopefully they are not necessary. That leaves the checksum: Let's hope Ubiquiti
used the same checksum algorithm like for the first block.
