...
Now to the script. That one is really a lot of stuff.
...
upload the config via the web interface. Reboot the router and go to
http://192.168.1.1:8080/lua. After that an unsigned firmware image will be
accepted. Only problem left: Generating the right firmware image.
