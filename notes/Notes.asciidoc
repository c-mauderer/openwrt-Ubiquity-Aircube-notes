...
option lua_handler '/tmp/persistent/config/lua_handler.lua'
----

Now every time +http://192.168.1.1/lua+ is opened the script +lua_handler.lua+
...
function handle_request(env)
    ...
    local command = "echo injected command"
...
The +local command+ can be replaced by any shell command.
